...
export async function GET(req: NextRequest) {
  ...
  if (appid) {
    ...
    const [allLocale, enLocale, ukLocale] = await Promise.all([
      payload.find({
        collection: "games",
        where: { appid: { equals: Number(appid) } },
        locale: "all",
        limit: 1,
      }),
      payload.find({
  ...
  if (q) {
    ...
    return NextResponse.json({
      query: q,
      en: {
        total: enResult.totalDocs,
        games: enResult.docs.map((g) => ({ appid: g.appid, name: g.name })),
      },
      uk: {
        total: ukResult.totalDocs,
        games: ukResult.docs.map((g) => ({ appid: g.appid, name: g.name })),
      },
    });
  }
  ...
  if (sp.get("migrate") === "true") {
    ...
    const collection =
      (db as any).collections["games"]?.collection ??
      (db as any).connection?.collection("games");
    if (!collection) {
      return NextResponse.json(
        { error: "Cannot access raw MongoDB collection" },
        { status: 500 },
      );
    }
    ...
    return NextResponse.json({
      ...
      sample: broken
        .slice(0, 5)
        .map((d: any) => ({ appid: d.appid, name: d.name })),
      status: "dry_run",
